defmodule PigLatin do
  ...
  def translate_word(word) do
    ...
    [c, v, tail] = String.split(word, ~r{(a|e|i|o|u|y)}, parts: 2, include_captures: true)
    ...
    cond do
      ...
      String.length(c) == 2 and (String.first(c) == "x" or String.first(c) == "y") and
          String.first(v) != "y" ->
        word <> "ay"
      ...
      String.length(c) == 2 and String.last(c) == "q" and String.first(v) == "u" ->
      ...
      String.length(c) == 1 and String.first(v) == "y" ->
        v <> tail <> c <> "ay"
